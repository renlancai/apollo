...
void BaseMap::InitMapNodeCaches(int cacheL1_size, int cahceL2_size) {
    if (map_node_cache_lvl1_ == NULL) {
        delete map_node_cache_lvl1_;
    }

    if (map_node_cache_lvl2_ == NULL) {
        delete map_node_cache_lvl2_;
    }
...
void BaseMap::LoadMapNodes(std::set<MapNodeIndex> &map_ids) {
    ...
    itr = map_ids.begin();
    while (itr != map_ids.end()) {
        p_map_load_threads_->schedule(boost::bind(
...
void BaseMap::PreloadMapNodes(std::set<MapNodeIndex> &map_ids) {
    ...
    while (itr != map_ids.end()) {
        ...
        lock3.unlock();
        p_map_preload_threads_->schedule(boost::bind(
                &BaseMap::LoadMapNodeThreadSafety, this, *itr, false));
        ++itr;
    }
}

void BaseMap::LoadMapNodeThreadSafety (
    const MapNodeIndex &index, bool is_reserved) {
...
void BaseMap::PreloadMapArea(const Eigen::Vector3d& location, const Eigen::Vector3d& trans_diff, 
                    unsigned int resolution_id, unsigned int zone_id) {
    if (p_map_preload_threads_ == NULL) {
        std::cerr << "Map preload thread is null!" << std::endl;
        return;
    }
    if (map_node_pool_ == NULL) {
        std::cerr << "Map node pool is null!" << std::endl;
        return;
    }
	...
	pt_top_left[2] = 0;
	MapNodeIndex map_id = MapNodeIndex::GetMapNodeIndex(*map_config_, pt_top_left, resolution_id, zone_id);
	...
	pt_top_right[0] = location[0] + (this->map_config_->map_node_size_x_ * map_pixel_resolution / 2.0);
	...
	pt_bottom_left[1] = location[1] + (this->map_config_->map_node_size_y_ * map_pixel_resolution / 2.0);
	pt_bottom_left[2] = 0;
    ...
    for (int i = -1; i < 2; ++i) {
        Eigen::Vector3d pt;
        pt[0] = location[0] + x_direction * 1.5 * this->map_config_->map_node_size_x_ * map_pixel_resolution;
        pt[1] = location[1] + static_cast<double>(i) * this->map_config_->map_node_size_y_ * map_pixel_resolution;
        pt[2] = 0;
    ...
    for (int i = -1; i < 2; ++i) {
        Eigen::Vector3d pt;
        pt[0] = location[0] + static_cast<double>(i) * this->map_config_->map_node_size_x_ * map_pixel_resolution;
        pt[1] = location[1] + y_direction * 1.5 * this->map_config_->map_node_size_y_ * map_pixel_resolution;
        pt[2] = 0;
        map_id = MapNodeIndex::GetMapNodeIndex(*map_config_, pt, resolution_id, zone_id);
        map_ids.insert(map_id);
    }
    {
        Eigen::Vector3d pt;
        pt[0] = location[0] + x_direction * 1.5 * this->map_config_->map_node_size_x_ 
            * map_pixel_resolution;
        pt[1] = location[1] + y_direction * 1.5 * this->map_config_->map_node_size_y_ 
            * map_pixel_resolution;
        pt[2] = 0;
        map_id = MapNodeIndex::GetMapNodeIndex(*map_config_, pt, resolution_id, zone_id);
        map_ids.insert(map_id);
...
bool BaseMap::LoadMapArea(const Eigen::Vector3d& seed_pt3d, unsigned int resolution_id, 
        unsigned int zone_id, int filter_size_x, int filter_size_y) {
    if (p_map_load_threads_ == NULL) {
        std::cerr << "Map load thread is null!" << std::endl;
        return false;
    }
    if (map_node_pool_ == NULL) {
        std::cerr << "Map node pool is null!" << std::endl;
	...
	///top left
	Eigen::Vector3d pt_top_left;
	pt_top_left[0] = seed_pt3d[0] - (this->map_config_->map_node_size_x_ * map_pixel_resolution / 2.0)
								  - static_cast<int>(filter_size_x / 2) * map_pixel_resolution;
	pt_top_left[1] = seed_pt3d[1] - (this->map_config_->map_node_size_y_ * map_pixel_resolution / 2.0)
	...
	///top right
	Eigen::Vector3d pt_top_right;
	pt_top_right[0] = seed_pt3d[0] + (this->map_config_->map_node_size_x_ * map_pixel_resolution / 2.0)
								  + static_cast<int>(filter_size_x / 2) * map_pixel_resolution;
	pt_top_right[1] = pt_top_left[1];
	pt_top_left[2] = 0;
	map_id = MapNodeIndex::GetMapNodeIndex(*map_config_, pt_top_right, resolution_id, zone_id);
	...
	Eigen::Vector3d pt_bottom_left;
	pt_bottom_left[0] = pt_top_left[0];
	pt_bottom_left[1] = seed_pt3d[1] + (this->map_config_->map_node_size_y_ * map_pixel_resolution / 2.0)
								     + static_cast<int>(filter_size_y / 2) * map_pixel_resolution;
	pt_bottom_left[2] = 0;
	map_id = MapNodeIndex::GetMapNodeIndex(*map_config_, pt_bottom_left, resolution_id, zone_id);
	map_ids.insert(map_id);
...
void BaseMap::ComputeMd5ForAllMapNodes() {
  ...
  for (unsigned int i = 0; i < all_map_node_paths_.size(); ++i) {
    std::string path = map_config_->map_folder_path_
                       + all_map_node_paths_[i];
    char md5[CHAR_MD5LENTH];
    ComputeFileMd5(path, md5);
