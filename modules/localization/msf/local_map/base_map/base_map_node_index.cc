...
MapNodeIndex MapNodeIndex::GetMapNodeIndex(const BaseMapConfig& option,
  ...
  if (n >= 0 && m >= 0 && n < GetMapIndexRangeEast(option, resolution_id) &&
   ...
   std::cerr << "MapNodeIndex::get_map_node_index illegal n: " 
     << n << ",m: " << m << std::endl;
...
unsigned int MapNodeIndex::GetMapIndexRangeEast(
...
unsigned int MapNodeIndex::GetMapIndexRangeNorth(
  const BaseMapConfig& option, 
  unsigned int resolution_id) {
  return static_cast<unsigned int>(
