...
AlignedMatrix<Scalar, aligned_len>::AlignedMatrix() : raw_ptr_(NULL), 
                    raw_size_(0), data_(NULL), row_data_(NULL), 
                    rows_(0), cols_(0) {
}
...
AlignedMatrix<Scalar, aligned_len>::AlignedMatrix(
    ...
    row_data_ = (Scalar**)malloc(sizeof(Scalar*) * rows_);
...
void AlignedMatrix<Scalar, aligned_len>::SetData(const Scalar* data, 
                        unsigned int data_size, unsigned int start_id) {
    memcpy(data_ + start_id, data, sizeof(Scalar) * data_size);                                                
}

template <typename Scalar, int aligned_len>
AlignedMatrix<Scalar, aligned_len>& AlignedMatrix<Scalar, aligned_len>::operator = (
        const AlignedMatrix<Scalar, aligned_len>& matrix) {
    if (raw_ptr_) {
        free(raw_ptr_);
        raw_size_ = 0;
