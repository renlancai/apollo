...
void PyramidMapMatrix::Init(unsigned int rows, unsigned int cols, 
    ...
    if (resolution_num < 1) {
        std::cerr << "PyramidMapMatrix: [init] The resolution_num should greater than 0." << std::endl;
        return;
    }

    // ratio should greater than 0
    if (ratio < 1) {
        std::cerr << "PyramidMapMatrix: [init] The ratio should greater than 0." << std::endl;
        return;
    }
    ...
    for (unsigned int i = 1; i < resolution_num_; ++i) {
        ...
        if (rows_remainder != 0 || cols_remainder != 0) {
            std::cerr << "PyramidMapMatrix: [init] "
                    << "Rows and cols in each level should be divisible by ratio." << std::endl;
            return;
        }
...
void PyramidMapMatrix::Reset(unsigned int level) {
    if (level >= resolution_num_) {
        std::cerr << "PyramidMapMatrix: [reset] The level id is illegal." << std::endl;
        return;
    }
...
void PyramidMapMatrix::ResetCells(unsigned int start_id, 
                                    unsigned int end_id,
                                    unsigned int level) {
    if (level >= resolution_num_) {
        std::cerr << "PyramidMapMatrix: [ResetCells] The level id is illegal." << std::endl;
        return;
    }
    ...
    if (start_id >= length || end_id >= length) {
        std::cerr 
            << "PyramidMapMatrix: [ResetCell] The start_id or end_id is illegal." << std::endl;
        return;
    }
...
void PyramidMapMatrix::ResetCell(unsigned int id, 
                                unsigned int level) {
...
bool PyramidMapMatrix::GetIntensityImg(unsigned int level, 
    ...
    if (!has_intensity_ || resolution_num_ < 1) {
        std::cerr 
            << "PyramidMapMatrix: [GetIntensityImg] No intensity data." << std::endl;
        return false;
    }

    if (level >= resolution_num_) {
        std::cerr << "PyramidMapMatrix: [GetIntensityImg] The level id is illegal." << std::endl;
        return false;
    }  
...
bool PyramidMapMatrix::GetAltitudeImg(unsigned int level, 
    ...
    if (!has_altitude_ || resolution_num_ < 1) {
        std::cerr 
            << "PyramidMapMatrix: [GetAltitudeImg] No altitude data." << std::endl;
        return false;
    }

    if (level >= resolution_num_) {
        std::cerr << "PyramidMapMatrix: [GetAltitudeImg] The level id is illegal." << std::endl;
        return false;
    }  
        ...
        for (unsigned int x = 0; x < cols_mr_[level]; x++) {        
            if (count_matrixes_[level][y][x] > 0) {
                if (altitude_matrixes_[level][y][x] >= max_alt) {
                    ...
                    altitude_img->at<unsigned char>(y, x) = 
                ...
                for (unsigned int rl = r0; rl < r0 + ratio_; ++rl) {
                    for (unsigned int cl = c0; cl < c0 + ratio_; ++cl) {
                        ...
                        if (count > 0) {
                            ...
                            GetMapCellSafe(intensity, intensity_var, altitude, altitude_var, 
                                        ground_altitude, count, ground_count, rl, cl, i-1);
                ...
                for (unsigned int rl = r0; rl < r0 + ratio_; ++rl) {
                    for (unsigned int cl = c0; cl < c0 + ratio_; ++cl) {
                        ...
                        if (count > 0) {
                            ...
                            GetMapCellBase(intensity, intensity_var, 
                                            altitude, count, rl, cl, i-1);
                            MergeCellBase(intensity, intensity_var, 
                                            altitude, count, r, c, i);
                        }
                    }
                }
            }
...
const float* PyramidMapMatrix::GetIntensitySafe(unsigned int row, 
    ...
    if (!has_intensity_) {
        std::cerr << "PyramidMapMatrix: [GetIntensitySafe] Has no intensity." << std::endl;
        return NULL;
    }

    if (!CheckLegalityForGetData(row, col, level)) {
        std::cerr << "PyramidMapMatrix: [GetIntensitySafe] Params is illegal." << std::endl;
        return NULL;
    }
...
const float* PyramidMapMatrix::GetIntensityVarSafe(unsigned int row, 
            ...
            unsigned int level) const {
    if (!has_intensity_var_) {
        std::cerr << "PyramidMapMatrix: [GetIntensityVarSafe] Has no intensity_var." << std::endl;
        return NULL;
    }

    if (!CheckLegalityForGetData(row, col, level)) {
        std::cerr << "PyramidMapMatrix: [GetIntensityVarSafe] Params is illegal." << std::endl;
        return NULL;
    }
...
const float* PyramidMapMatrix::GetAltitudeSafe(unsigned int row, 
            ...
            unsigned int level) const {
    if (!has_altitude_) {
        std::cerr << "PyramidMapMatrix: [GetAltitudeSafe] Has no altitude." << std::endl;
        return NULL;
    }

    if (!CheckLegalityForGetData(row, col, level)) {
        std::cerr << "PyramidMapMatrix: [GetAltitudeSafe] Params is illegal." << std::endl;
        return NULL;
    }
...
const float* PyramidMapMatrix::GetAltitudeVarSafe(unsigned int row, 
            ...
            unsigned int level) const {
    if (!has_altitude_var_) {
        std::cerr << "PyramidMapMatrix: [GetAltitudeVarSafe] Has no altitude_var." << std::endl;
        return NULL;
    }

    if (!CheckLegalityForGetData(row, col, level)) {
        std::cerr << "PyramidMapMatrix: [get_altitude_var] Params is illegal." << std::endl;
        return NULL;
    }
...
const float* PyramidMapMatrix::GetGroundAltitudeSafe(unsigned int row, 
            ...
            unsigned int level) const {
    if (!has_ground_altitude_) {
        std::cerr << "PyramidMapMatrix: [GetGroundAltitudeSafe] Has no ground_altitude." << std::endl;
        return NULL;
    }

    if (!CheckLegalityForGetData(row, col, level)) {
        std::cerr << "PyramidMapMatrix: [GetGroundAltitudeSafe] Params is illegal." << std::endl;
        return NULL;
    }
...
const unsigned int* PyramidMapMatrix::GetCountSafe(unsigned int row, 
    ...
    if (!CheckLegalityForGetData(row, col, level)) {
        std::cerr << "PyramidMapMatrix: [GetCountSafe] Params is illegal." << std::endl;
        return NULL;
    }
...
const unsigned int* PyramidMapMatrix::GetGroundCountSafe(unsigned int row, 
            unsigned int col,
            unsigned int level) const {
    if (!has_ground_count_) {
        std::cerr << "PyramidMapMatrix: [GetGroundCountSafe] Has no ground_count." << std::endl;
        return NULL;
    }

    if (!CheckLegalityForGetData(row, col, level)) {
        std::cerr << "PyramidMapMatrix: [GetGroundCountSafe] Params is illegal." << std::endl;
        return NULL;
    }
...
void PyramidMapMatrix::GetMapCellSafe(float* &intensity, float* &intensity_var,
    ...
    if (!CheckLegalityForGetData(row, col, level)) {
        std::cerr << "PyramidMapMatrix: [GetMapCellSafe] Params is illegal." << std::endl;
        return;
    }
...
FloatMatrix* PyramidMapMatrix::GetIntensityMatrixSafe(
        unsigned int level) {
    if (!has_intensity_) {
        std::cerr << "PyramidMapMatrix: [GetIntensityMatrixSafe] Has no intensity." << std::endl;
        return NULL;
    }         

    if (level >= resolution_num_) {
        std::cerr << "PyramidMapMatrix: [GetIntensityMatrixSafe] The level id is illegal." << std::endl;
        return NULL;
    } 
...
FloatMatrix* PyramidMapMatrix::GetIntensityVarMatrixSafe(
        unsigned int level) {
    if (!has_intensity_var_) {
        std::cerr << "PyramidMapMatrix: [GetIntensityVarMatrixSafe] Has no intensity_var." << std::endl;
        return NULL;
    }

    if (level >= resolution_num_) {
        std::cerr << "PyramidMapMatrix: [GetIntensityVarMatrixSafe] The level id is illegal." << std::endl;
        return NULL;
    } 
...
FloatMatrix* PyramidMapMatrix::GetAltitudeMatrixSafe(
        unsigned int level) {
    if (!has_altitude_) {
        std::cerr << "PyramidMapMatrix: [GetAltitudeMatrixSafe] Has no altitude." << std::endl;
        return NULL;
    }

    if (level >= resolution_num_) {
        std::cerr << "PyramidMapMatrix: [GetAltitudeMatrixSafe] The level id is illegal." << std::endl;
        return NULL;
    } 
...
FloatMatrix* PyramidMapMatrix::GetAltitudeVarMatrixSafe(
        unsigned int level) {
    if (!has_altitude_var_) {
        std::cerr << "PyramidMapMatrix: [GetAltitudeVarMatrixSafe] Has no altitude_var." << std::endl;
        return NULL;
    }

    if (level >= resolution_num_) {
        std::cerr << "PyramidMapMatrix: [GetAltitudeVarMatrixSafe] The level id is illegal." << std::endl;
        return NULL;
    } 
...
FloatMatrix* PyramidMapMatrix::GetGroundAltitudeMatrixSafe(
        unsigned int level) {
    if (!has_ground_altitude_) {
        std::cerr << "PyramidMapMatrix: [GetGroundAltitudeMatrixSafe] Has no ground_altitude." << std::endl;
        return NULL;
    }

    if (level >= resolution_num_) {
        std::cerr << "PyramidMapMatrix: [GetGroundAltitudeMatrixSafe] The level id is illegal." << std::endl;
        return NULL;
    } 
...
UIntMatrix* PyramidMapMatrix::GetCountMatrixSafe(
        unsigned int level) {
    if (!has_count_) {
        std::cerr << "PyramidMapMatrix: [GetCountMatrixSafe] Has no count." << std::endl;
        return NULL;
    }

    if (level >= resolution_num_) {
        std::cerr << "PyramidMapMatrix: [GetCountMatrixSafe] The level id is illegal." << std::endl;
        return NULL;
    } 
...
UIntMatrix* PyramidMapMatrix::GetGroundCountMatrixSafe(
        unsigned int level) {
    if (!has_ground_count_) {
        std::cerr << "PyramidMapMatrix: [GetGroundCountMatrixSafe] Has no ground_count." << std::endl;
        return NULL;
    }

    if (level >= resolution_num_) {
        std::cerr << "PyramidMapMatrix: [GetGroundCountMatrixSafe] The level id is illegal." << std::endl;
        return NULL;
    } 
...
void PyramidMapMatrix::SetIntensityMatrix(
        const float* input, unsigned int size, unsigned int start_index,
        unsigned int level) {
    if (!has_intensity_) {
        std::cerr << "PyramidMapMatrix: [SetIntensityMatrix] Has no intensity." << std::endl;
        return;
    }

    if (!CheckLegalityForSetData(level, start_index, size)) {
        std::cerr << "PyramidMapMatrix: [SetIntensityMatrix] Params is illegal." << std::endl;
        return;
    }
...
void PyramidMapMatrix::SetIntensityVarMatrix(
        const float* input, unsigned int size, unsigned int start_index,
        unsigned int level) {
    if (!has_intensity_var_) {
        std::cerr << "PyramidMapMatrix: [set_intensity_var_matrix] Has no intensity_var." << std::endl;
        return;
    }

    if (!CheckLegalityForSetData(level, start_index, size)) {
        std::cerr << "PyramidMapMatrix: [set_intensity_var_matrix] Params is illegal." << std::endl;
        return;
    }
...
void PyramidMapMatrix::SetAltitudeMatrix(
        const float* input, unsigned int size, unsigned int start_index,
        unsigned int level) {
    if (!has_altitude_) {
        std::cerr << "PyramidMapMatrix: [SetAltitudeMatrix] Has no altitude." << std::endl;
        return;
    }

    if (!CheckLegalityForSetData(level, start_index, size)) {
        std::cerr << "PyramidMapMatrix: [SetAltitudeMatrix] Params is illegal." << std::endl;
        return;
    }
...
void PyramidMapMatrix::SetAltitudeVarMatrix(
        const float* input, unsigned int size, unsigned int start_index,
        unsigned int level) {
    if (!has_altitude_var_) {
        std::cerr << "PyramidMapMatrix: [SetAltitudeVarMatrix] Has no altitude_var." << std::endl;
        return;
    }

    if (!CheckLegalityForSetData(level, start_index, size)) {
        std::cerr << "PyramidMapMatrix: [SetAltitudeVarMatrix] Params is illegal." << std::endl;
        return;
    }
...
void PyramidMapMatrix::SetGroundAltitudeMatrix(
        const float* input, unsigned int size, unsigned int start_index,
        unsigned int level) {
    if (!has_ground_altitude_) {
        std::cerr << "PyramidMapMatrix: [SetGroundAltitudeMatrix] Has no ground_altitude." << std::endl;
        return;
    }

    if (!CheckLegalityForSetData(level, start_index, size)) {
        std::cerr << "PyramidMapMatrix: [SetGroundAltitudeMatrix] Params is illegal." << std::endl;
        return;
    }
...
void PyramidMapMatrix::SetCountMatrix(
        const unsigned int* input, unsigned int size, unsigned int start_index,
        unsigned int level) {
    if (!has_count_) {
        std::cerr << "PyramidMapMatrix: [SetCountMatrix] Has no count." << std::endl;
        return;
    }

    if (!CheckLegalityForSetData(level, start_index, size)) {
        std::cerr << "PyramidMapMatrix: [SetCountMatrix] Params is illegal." << std::endl;
        return;
    }
...
void PyramidMapMatrix::SetGroundCountMatrix(
        const unsigned int* input, unsigned int size, unsigned int start_index,
        unsigned int level) {
    if (!has_ground_count_) {
        std::cerr << "PyramidMapMatrix: [SetGroundCountMatrix] Has no ground count." << std::endl;
        return;
    }

    if (!CheckLegalityForSetData(level, start_index, size)) {
        std::cerr << "PyramidMapMatrix: [SetGroundCountMatrix] Params is illegal." << std::endl;
        return;
    }
...
void PyramidMapMatrix::SetFloatMatrixRoi(
        const FloatMatrix* source_matrix, 
        const Rect2D<unsigned int> &source_roi, 
        const Rect2D<unsigned int> &target_roi, 
        unsigned int type,
        unsigned int level) {
    if (source_matrix == NULL) {
        std::cerr << "PyramidMapMatrix: [SetFloatMatrixRoi] Source matrix is null." << std::endl;
        return;
    }
    ...
    switch (type) {
        case 0:
            if (!has_intensity_) {
                std::cerr << "PyramidMapMatrix: [SetFloatMatrixRoi] Has no intensity." << std::endl;
                return;
            }
            break;
        case 1:
            if (!has_intensity_var_) {
                std::cerr << "PyramidMapMatrix: [SetFloatMatrixRoi] Has no intensity var." << std::endl;
                return;
            }
            break;
        case 2:
            if (!has_altitude_) {
                std::cerr << "PyramidMapMatrix: [SetFloatMatrixRoi] Has no altitude." << std::endl;
                return;
            }
            break;
        case 3:
            if (!has_altitude_var_) {
                std::cerr << "PyramidMapMatrix: [SetFloatMatrixRoi] Has no altitude var." << std::endl;
                return;
            }
            break;
        case 4:
            if (!has_ground_altitude_) {
                std::cerr << "PyramidMapMatrix: [SetFloatMatrixRoi] Has no ground altitude." << std::endl;
                return;
            }
            break;
    }

    if (!CheckLegalityForSetDataRoi(level, 
        static_cast<unsigned int>(source_matrix->GetRow()), 
        static_cast<unsigned int>(source_matrix->GetCol()), 
        source_roi, target_roi)) {
        std::cerr << "PyramidMapMatrix: [SetFloatMatrixRoi] Params is illegal." << std::endl;
        return;
    }
    ...
    for (unsigned int y = target_roi_min_y; y <= target_roi_max_y; ++y) {
        ...
        unsigned int source_start_index = (source_roi_min_y + inc) * 
                                    source_matrix->GetCol() + source_roi_min_x;
...
void PyramidMapMatrix::SetUintMatrixRoi(
        const UIntMatrix* source_matrix, 
        const Rect2D<unsigned int> &source_roi, 
        const Rect2D<unsigned int> &target_roi, 
        unsigned int type,
        unsigned int level) {
    if (source_matrix == NULL) {
        std::cerr << "PyramidMapMatrix: [SetUintMatrixRoi] Source matrix is null." << std::endl;
        return;
    }
    ...
    switch (type) {
        case 0:
            if (!has_count_) {
                std::cerr << "PyramidMapMatrix: [SetUintMatrixRoi] Has no count." << std::endl;
                return;
            }
            break;
        case 1:
            if (!has_ground_count_) {
                std::cerr << "PyramidMapMatrix: [SetUintMatrixRoi] Has no ground count." << std::endl;
                return;
            }
            break;
    }
    ...
    if (!CheckLegalityForSetDataRoi(level, 
        ...
        static_cast<unsigned int>(source_matrix->GetCol()), 
        source_roi, target_roi)) {
        std::cerr << "PyramidMapMatrix: [SetUintMatrixRoi] Params is illegal." << std::endl;
        return;
    }
...
void PyramidMapMatrix::SetIntensitySafe(float intensity, unsigned int row, 
            unsigned int col, unsigned int level) {
    if (!has_intensity_) {
        std::cerr << "PyramidMapMatrix: [SetIntensitySafe] Has no intensity." << std::endl;
        return;
    }

    if (!CheckLegalityForGetData(row, col, level)) {
        std::cerr << "PyramidMapMatrix: [SetIntensitySafe] Params is illegal." << std::endl;
        return;
    }
...
void PyramidMapMatrix::SetIntensityVarSafe(float intensity_var, unsigned int row, 
            unsigned int col, unsigned int level) {
    if (!has_intensity_var_) {
        std::cerr << "PyramidMapMatrix: [SetIntensityVarSafe] Has no intensity_var." << std::endl;
        return;
    }

    if (!CheckLegalityForGetData(row, col, level)) {
        std::cerr << "PyramidMapMatrix: [SetIntensityVarSafe] Params is illegal." << std::endl;
        return;
    }
...
void PyramidMapMatrix::SetAltitudeSafe(float altitude, unsigned int row, 
            unsigned int col, unsigned int level) {
    if (!has_altitude_) {
        std::cerr << "PyramidMapMatrix: [SetAltitudeSafe] Has no altitude." << std::endl;
        return;
    }

    if (!CheckLegalityForGetData(row, col, level)) {
        std::cerr << "PyramidMapMatrix: [SetAltitudeSafe] Params is illegal." << std::endl;
        return;
    }
...
void PyramidMapMatrix::SetAltitudeVarSafe(float altitude_var, unsigned int row, 
            unsigned int col, unsigned int level) {
    if (!has_altitude_var_) {
        std::cerr << "PyramidMapMatrix: [SetAltitudeVarSafe] Has no altitude var." << std::endl;
        return;
    }

    if (!CheckLegalityForGetData(row, col, level)) {
        std::cerr << "PyramidMapMatrix: [SetAltitudeVarSafe] Params is illegal." << std::endl;
        return;
    }
...
void PyramidMapMatrix::SetGroundAltitudeSafe(float ground_altitude, unsigned int row, 
            unsigned int col, unsigned int level) {
    if (!has_ground_altitude_) {
        std::cerr << "PyramidMapMatrix: [SetGroundAltitudeSafe] Has no ground altitude." << std::endl;
        return;
    }

    if (!CheckLegalityForGetData(row, col, level)) {
        std::cerr << "PyramidMapMatrix: [SetGroundAltitudeSafe] Params is illegal." << std::endl;
        return;
    }
...
void PyramidMapMatrix::SetCountSafe(unsigned int count, unsigned int row, 
    ...
    if (!CheckLegalityForGetData(row, col, level)) {
        std::cerr << "PyramidMapMatrix: [SetCountSafe] Params is illegal." << std::endl;
        return;
    }
...
void PyramidMapMatrix::SetGroundCountSafe(unsigned int ground_count, unsigned int row, 
            unsigned int col, unsigned int level) {
    if (!has_ground_count_) {
        std::cerr << "PyramidMapMatrix: [SetGroundCountSafe] Has no ground count." << std::endl;
        return;
    }

    if (!CheckLegalityForGetData(row, col, level)) {
        std::cerr << "PyramidMapMatrix: [SetGroundCountSafe] Params is illegal." << std::endl;
        return;
    }
...
void PyramidMapMatrix::SetValueSafe(unsigned char intensity, float altitude,
                    ...
                    unsigned int level) {
    if (!has_intensity_) {
        std::cerr << "PyramidMapMatrix: [SetValueSafe] Has no intensity." << std::endl;
        return;
    }

    if (!has_altitude_) {
        std::cerr << "PyramidMapMatrix: [SetValueSafe] Has no altitude." << std::endl;
        return;
    }

    if (!CheckLegalityForGetData(row, col, level)) {
        std::cerr << "PyramidMapMatrix: [SetValueSafe] Params is illegal." << std::endl;
        return;
    }
...
void PyramidMapMatrix::MergeCellSafe(const float* intensity,
                    ...
                    unsigned int level) {
    if (!CheckLegalityForGetData(row, col, level)) {
        std::cerr << "PyramidMapMatrix: [MergeCellSafe] Params is illegal." << std::endl;
        return;
    }
    ...
    float p0 = static_cast<float>(count_matrixes_[level][row][col]) / new_count;
    float p1 = static_cast<float>(*count) / new_count;
    ...
    if (intensity != NULL && has_intensity_ && 
        intensity_var != NULL && has_intensity_var_) {
        intensity_var_matrixes_[level][row][col] = 
            intensity_var_matrixes_[level][row][col] * p0
            + *intensity_var * p1 + intensity_diff * intensity_diff * p0 * p1;
    }
    ...
    if (altitude != NULL && has_altitude_ && 
        altitude_var != NULL && has_altitude_var_) {
        altitude_var_matrixes_[level][row][col] = 
            altitude_var_matrixes_[level][row][col] * p0
            + *altitude_var * p1 + altitude_diff * altitude_diff * p0 * p1;
    }
    ...
    p0 = static_cast<float>(ground_count_matrixes_[level][row][col]) / new_ground_count;
    p1 = static_cast<float>(*ground_count) / new_ground_count; 

    float ground_altitude_diff = 0.0f;
    if (ground_altitude != NULL && has_ground_altitude_) {
        ground_altitude_diff 
            = ground_altitude_matrixes_[level][row][col] - *ground_altitude;
...
bool PyramidMapMatrix::CheckLegalityForGetData(unsigned int row,
                        ...
                        unsigned int level) const{
    if (level >= resolution_num_) {
        std::cerr << "PyramidMapMatrix: [CheckLegalityForGetData] The level id is illegal." << std::endl;
        return false;
    } 

    if (row >= rows_mr_[level]) {
        std::cerr << "PyramidMapMatrix: [CheckLegalityForGetData] The row is illegal." << std::endl;
        return false;
    } 

    if (col >= cols_mr_[level]) {
        std::cerr << "PyramidMapMatrix: [CheckLegalityForGetData] The col is illegal." << std::endl;
        return false;
    }
...
bool PyramidMapMatrix::CheckLegalityForSetData(unsigned int level,
    ...
    if (level >= resolution_num_) {
        std::cerr << "PyramidMapMatrix: [CheckLegalityForSetData] The level id is illegal." << std::endl;
        return false;
    }

    if (start_id + size > rows_mr_[level] * cols_mr_[level]) {
        std::cerr << "PyramidMapMatrix: [CheckLegalityForSetData] The start_id or size is illegal." << std::endl;
        return false;
    }
...
bool PyramidMapMatrix::CheckLegalityForSetDataRoi(unsigned int level,
    ...
    if (level >= resolution_num_) {
        std::cerr << "PyramidMapMatrix: [CheckLegalityForSetDataRoi] The level id is illegal." << std::endl;
        return false;
    }
    ...
    if (source_roi_min_x > source_roi_max_x || 
       ...
       target_roi_max_y - target_roi_min_y) {
        std::cerr << "PyramidMapMatrix: [CheckLegalityForSetDataRoi]" 
                " The source_roi or target_roi is illegal." << std::endl;
        return false;
    }
...
void PyramidMapMatrix::AddSampleSafe(float intensity, 
                        float altitude,
                        unsigned int row,
                        unsigned int col,
                        unsigned int level) {
    if (!CheckLegalityForGetData(row, col, level)) {
        std::cerr << "PyramidMapMatrix: [AddSampleSafe] Params is illegal." << std::endl;
        return;
    }
    ...
    if (has_count_ && has_intensity_) {
        ...
        value = v1 / count_matrixes_[level][row][col];
    ...
    if (has_count_ && has_intensity_ && has_intensity_var_) {
        ...
        intensity_var_matrixes_[level][row][col] 
            ...
            * intensity_var_matrixes_[level][row][col] + v1 * v2) 
            / count_matrixes_[level][row][col];
    }
    ...
    if (has_count_ && has_altitude_ && has_altitude_var_) {
        ...
        altitude_var_matrixes_[level][row][col] 
            = (static_cast<float>(count_matrixes_[level][row][col] - 1) 
            * altitude_var_matrixes_[level][row][col] + v1 * v2) 
            / count_matrixes_[level][row][col];
    }
}
...
void PyramidMapMatrix::AddGroundSample(float ground_altitude,
            ...
            unsigned int level) {
    if (!CheckLegalityForGetData(row, col, level)) {
        std::cerr << "PyramidMapMatrix: [AddGroundSample] Params is illegal." << std::endl;
        return;
    }
    ...
    if (has_ground_count_ && has_ground_altitude_) {
        ...
        value = v1 / ground_count_matrixes_[level][row][col];
...
double PyramidMapMatrix::ComputeMeanIntensity(unsigned int level) {
    if (!has_count_) {
        std::cerr << "PyramidMapMatrix: [ComputeMeanIntensity] Has no count." << std::endl;
        return 0.0;
    }

    if (!has_intensity_ || resolution_num_ < 1) {
        std::cerr 
            << "PyramidMapMatrix: [ComputeMeanIntensity] No intensity data." << std::endl;
        return 0.0;
    }

    if (level >= resolution_num_) {
        std::cerr << "PyramidMapMatrix: [ComputeMeanIntensity] The level id is illegal." << std::endl;
        return 0.0;
    }  
...
void PyramidMapMatrix::Reduce(PyramidMapMatrix* cells, 
    ...
    if (level >= cells->resolution_num_) {
        std::cerr << "PyramidMapMatrix: [Reduce] The level id is illegal." << std::endl;
        return;
    }  

    if (new_level >= new_cells.resolution_num_) {
        std::cerr << "PyramidMapMatrix: [Reduce] The new level id is illegal." << std::endl;
        return;
    }      
    ...
    for (unsigned int r = 0; r < cells->rows_mr_[level]; r++) {
        for (unsigned int c = 0; c < cells->cols_mr_[level]; c++) {
                const float* intensity = 
                    new_cells.GetIntensitySafe(r, c, new_level);
                const float* intensity_var = 
                    new_cells.GetIntensityVarSafe(r, c, new_level);
                const float* altitude = 
                    new_cells.GetAltitudeSafe(r, c, new_level);
                const float* altitude_var = 
                    new_cells.GetAltitudeVarSafe(r, c, new_level);
                const float* ground_altitude = 
                    new_cells.GetGroundAltitudeSafe(r, c, new_level);
                const unsigned int* count = 
                    new_cells.GetCountSafe(r, c, new_level);
                const unsigned int* ground_count = 
                    new_cells.GetGroundCountSafe(r, c, new_level); 

                cells->MergeCellSafe(intensity, intensity_var,
                    altitude, altitude_var, ground_altitude,
                    count, ground_count, r, c, level);
        }
    }
}
