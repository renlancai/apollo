...
unsigned int LossyMapFullAltMatrixHandler::CreateBinary(
  ...
  if (buf_size >= target_size) {
    ...
    *float_p = ground_alt_max_;
    ...
    for (unsigned int r = 0; r < rows; ++r) {
      for (unsigned int c = 0; c < cols; ++c) {
        const float* intensity = matrix->GetIntensitySafe(r, c);
    ...
    for (unsigned int r = 0; r < rows; ++r) {
      for (unsigned int c = 0; c < cols; ++c) {
        const float* intensity_var = matrix->GetIntensityVarSafe(r, c);
...
unsigned int LosslessMapMatrixHandler::CreateBinary(
    const BaseMapMatrix* base_matrix, unsigned char* buf,
    unsigned int buf_size) {
  const PyramidMapMatrix* matrix =
      dynamic_cast<const PyramidMapMatrix*>(base_matrix);
...
unsigned int LosslessMapMatrixHandler::GetBinarySize(
    const BaseMapMatrix* base_matrix) {
  const PyramidMapMatrix* matrix =
      dynamic_cast<const PyramidMapMatrix*>(base_matrix);
  // assert(matrix->get_resolution_num() > 0);
...
unsigned int PyramidLossyMapMatrixHandler::LoadBinary(
    const unsigned char* buf, BaseMapMatrix* base_matrix) {
  PyramidMapMatrix* matrix = dynamic_cast<PyramidMapMatrix*>(base_matrix);
...
unsigned int PyramidLossyMapMatrixHandler::CreateBinary(
    const BaseMapMatrix* base_matrix, unsigned char* buf,
    unsigned int buf_size) {
  const PyramidMapMatrix* matrix =
      dynamic_cast<const PyramidMapMatrix*>(base_matrix);
