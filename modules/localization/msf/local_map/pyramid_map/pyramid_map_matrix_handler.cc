...
unsigned char LossyMapMatrixHandler::EncodeIntensity(
...
void LossyMapMatrixHandler::DecodeIntensity(
                unsigned char data, float *intensity) const {
    *intensity = static_cast<float>(data);
}

unsigned short LossyMapMatrixHandler::EncodeIntensityVar(float var) const {
    var = std::sqrt(var);
    int encoded_var = var_range_ / (var * var_ratio_ + 1.0);
    ...
    return static_cast<unsigned short>(encoded_var);
}

void LossyMapMatrixHandler::DecodeIntensityVar(unsigned short data, 
                                                        float *var) const {
    *var = data;
...
unsigned short LossyMapMatrixHandler::EncodeAltitude(
        float altitude, float min_altitude, float altitude_interval) const {
    float delta_alt = altitude - min_altitude;
    delta_alt /= altitude_interval;
    int encoded_altitude = static_cast<int>(delta_alt + 0.5);
    ...
    return static_cast<unsigned short>(encoded_altitude);
}

void LossyMapMatrixHandler::DecodeAltitude(unsigned short data,
        float min_altitude, float altitude_interval, float *altitude) const {
...
void LossyMapMatrixHandler::DecodeCount(
                                    unsigned char data, 
...
unsigned int LossyMapFullAltMatrixHandler::LoadBinary(
                                        const unsigned char* buf, 
                                        BaseMapMatrix* base_matrix) {
    PyramidMapMatrix* matrix = dynamic_cast<PyramidMapMatrix*>(base_matrix); 

    unsigned int binary_size = sizeof(unsigned int) * 2;
    ...
    if (// matrix->get_resolution_num() == 1 && 
        ...
        cols == matrix->GetCols() && 
        matrix->HasIntensity() && 
        matrix->HasIntensityVar() &&
        matrix->HasAltitude() &&
        matrix->HasGroundAltitude() &&
        matrix->HasCount()) {
        matrix->Reset();    
    }
    else {
        matrix->Init(rows, cols, true, true, true, 
    ...
    if (matrix->HasIntensity()) {
        binary_size += sizeof(unsigned char) *  matrix_size;
        FloatMatrix* intensity_matrix = 
                    matrix->GetIntensityMatrixSafe();
        for (unsigned int r = 0; r < rows; ++r) {
            for (unsigned int c = 0; c < cols; ++c) {
    ...
    if (matrix->HasIntensityVar()) {
        binary_size += sizeof(unsigned short) *  matrix_size;
        FloatMatrix* intensity_var_matrix = 
                    matrix->GetIntensityVarMatrixSafe();
        const unsigned char *p_low = uc_p + matrix_size;
        const unsigned char *p_high = uc_p;
        for (unsigned int r = 0; r < rows; ++r) {
            for (unsigned int c = 0; c < cols; ++c) {
                unsigned short var = p_high[r * cols + c];
    ...
    if (matrix->HasAltitude()) {
        binary_size += sizeof(unsigned short) *  matrix_size;
        FloatMatrix* altitude_matrix = 
                    matrix->GetAltitudeMatrixSafe();
        const unsigned char *p_low = uc_p + matrix_size;
        const unsigned char *p_high = uc_p;
        for (unsigned int r = 0; r < rows; ++r) {
            for (unsigned int c = 0; c < cols; ++c) {
                unsigned short alt = p_high[r * cols + c];
                alt = alt * 256 + p_low[r * cols + c];
                DecodeAltitude(alt, alt_avg_min_, 
    ...
    if (matrix->HasGroundAltitude()) {
        ...
        FloatMatrix* ground_altitude_matrix = 
                matrix->GetGroundAltitudeMatrixSafe();
        const unsigned char *p_low = uc_p + matrix_size;
        const unsigned char *p_high = uc_p;
        for (unsigned int r = 0; r < rows; ++r) {
            for (unsigned int c = 0; c < cols; ++c) {
                unsigned short alt = p_high[r * cols + c];
                alt = alt * 256 + p_low[r * cols + c];
                DecodeAltitude(alt, ground_alt_min_, 
...
unsigned int LossyMapFullAltMatrixHandler::CreateBinary(
    ...
    unsigned int target_size = GetBinarySize(matrix);
        ...
        *float_p = ground_alt_max_;
        ...
        for (unsigned int r = 0; r < rows; ++r) {
            for (unsigned int c = 0; c < cols; ++c) {
                ...
                unsigned short var = EncodeIntensityVar(iv);
                p_high[r * cols + c] = var / 256;
                p_low[r * cols + c] = var % 256;
            }
        }
        uc_p += 2 * matrix_size;
        // processed_size += matrix_size * sizeof(unsigned short);
        ...
        for (unsigned int r = 0; r < rows; ++r) {
            for (unsigned int c = 0; c < cols; ++c) {
                const float *altitude = 
                            matrix->GetAltitudeSafe(r, c);
                float a = (altitude != NULL) ? *altitude : 0.0; 
                unsigned short alt = EncodeAltitude(a, 
                            alt_avg_min_, alt_avg_interval_);
        ...
        for (unsigned int r = 0; r < rows; ++r) {
            for (unsigned int c = 0; c < cols; ++c) {
                const float *ground_altitude = 
                            matrix->GetGroundAltitudeSafe(r, c);
                float ga = (ground_altitude != NULL) ? *ground_altitude : 0.0; 
                unsigned short alt = EncodeAltitude(ga, 
                            ground_alt_min_, ground_alt_interval_);
                p_high[r * cols + c] = alt / 256;
                p_low[r * cols + c] = alt % 256;
            }
        }
...
unsigned int LossyMapFullAltMatrixHandler::GetBinarySize(
                                const BaseMapMatrix* base_matrix) {
    const PyramidMapMatrix* matrix = 
                dynamic_cast<const PyramidMapMatrix*>(base_matrix);                                
    ...
    unsigned int target_size = sizeof(unsigned int)*2 + sizeof(float)*4; 
...
unsigned int LosslessMapMatrixHandler::LoadBinary(const unsigned char* buf, 
                                                    BaseMapMatrix* base_matrix) {
    PyramidMapMatrix* matrix = dynamic_cast<PyramidMapMatrix*>(base_matrix);                                                    

    unsigned int binary_size = sizeof(unsigned int) * 2; // rows and cols
    ...
    if (// matrix->get_resolution_num() == 1 && 
        // matrix->get_resolution_ratio() == 2 &&
        rows == matrix->GetRowsSafe() && 
        cols == matrix->GetColsSafe() && 
        matrix->HasIntensity() && 
        matrix->HasIntensityVar() &&
        matrix->HasAltitude() &&
        matrix->HasAltitudeVar() &&
        matrix->HasGroundAltitude() &&
        matrix->HasCount() &&
...
unsigned int LosslessMapMatrixHandler::CreateBinary(const BaseMapMatrix* base_matrix, 
        ...
        for (unsigned int y = 0; y < rows; y++) {
            for (unsigned int x = 0; x < cols; x++) {
                ...
                for (unsigned int i = 0; i < layer_num; i++) {
                    ...
                    // unsigned int processed_size += sizeof(float) * 4 + sizeof(unsigned int);
                    // assert(buf_size >= processed_size);
                    // buf_size -= processed_size;
                }
            }
...
unsigned int LosslessMapMatrixHandler::GetBinarySize(const BaseMapMatrix* base_matrix) {
    const PyramidMapMatrix* matrix = 
                dynamic_cast<const PyramidMapMatrix*>(base_matrix);
    // assert(matrix->get_resolution_num() > 0);
    if (matrix->GetResolutionNum() <= 0) {
...
unsigned int PyramidLossyMapMatrixHandler::LoadBinary(const unsigned char* buf, 
                                                    BaseMapMatrix* base_matrix) {
    PyramidMapMatrix* matrix = dynamic_cast<PyramidMapMatrix*>(base_matrix);

    unsigned int binary_size = sizeof(unsigned int) * 4;
    ...
    if (resolution_num == matrix->GetResolutionNum() && 
        ratio == matrix->GetResolutionRatio() &&
        rows == matrix->GetRowsSafe() && 
        cols == matrix->GetColsSafe() && 
    ...
    for (unsigned int l = 0; l < resolution_num; ++l) {
        ...
        if (matrix->HasGroundCount()) {
            binary_size += sizeof(unsigned char) *  matrix_size;
            UIntMatrix* ground_count_matrix = 
        ...
        if (matrix->HasIntensity()) {
            binary_size += sizeof(unsigned char) *  matrix_size;
            FloatMatrix* intensity_matrix = 
                        matrix->GetIntensityMatrixSafe(l);
            for (unsigned int r = 0; r < rows; ++r) {
                for (unsigned int c = 0; c < cols; ++c) {
                    DecodeIntensity(uc_p[r * cols + c], &(*intensity_matrix)[r][c]); 
                }
        ...
        if (matrix->HasAltitude()) {
            binary_size += sizeof(unsigned short) *  matrix_size;
            FloatMatrix* altitude_matrix = 
                        matrix->GetAltitudeMatrixSafe(l);
            const unsigned char *p_low = uc_p + matrix_size;
            const unsigned char *p_high = uc_p;
            for (unsigned int r = 0; r < rows; ++r) {
                for (unsigned int c = 0; c < cols; ++c) {
...
unsigned int PyramidLossyMapMatrixHandler::CreateBinary(const BaseMapMatrix* base_matrix, 
                                    unsigned char * buf, unsigned int buf_size) {
    const PyramidMapMatrix* matrix = 
                dynamic_cast<const PyramidMapMatrix*>(base_matrix);
        ...
        for (unsigned int l = 0; l < resolution_num; ++l) {
            ...
            if (matrix->HasCount()) {
                for (unsigned int r = 0; r < rows; ++r) {
                    for (unsigned int c = 0; c < cols; ++c) {
                        ...
                        uc_p[r * cols + c] = EncodeCount(*count, 
                                            count_range_);
                    }
                }
                uc_p += matrix_size;
            ...
            if (matrix->HasGroundCount()) {
                for (unsigned int r = 0; r < rows; ++r) {
                    for (unsigned int c = 0; c < cols; ++c) {
                        ...
                        uc_p[r * cols + c] = EncodeCount(*ground_count, 
                                                        count_range_);
                    }
                }
                uc_p += matrix_size;
            ...
            if (matrix->HasIntensityVar()) {
                ...
                for (unsigned int r = 0; r < rows; ++r) {
                    for (unsigned int c = 0; c < cols; ++c) {
                        const float *intensity_var = 
                                    matrix->GetIntensityVarSafe(r, c, l);
                        unsigned short var = EncodeIntensityVar(*intensity_var);
                        p_high[r * cols + c] = var / 256;
                        p_low[r * cols + c] = var % 256;
                    }
                }
            ...
            if (matrix->HasAltitude()) {
                ...
                for (unsigned int r = 0; r < rows; ++r) {
                    for (unsigned int c = 0; c < cols; ++c) {
                        const float *altitude = 
                                    matrix->GetAltitudeSafe(r, c, l);
                        unsigned short alt = EncodeAltitude(*altitude, 
                                    alt_avg_min_, alt_avg_interval_);
                        p_high[r * cols + c] = alt / 256;
                        p_low[r * cols + c] = alt % 256;
                    }
            ...
            if (matrix->HasGroundAltitude()) {
                ...
                for (unsigned int r = 0; r < rows; ++r) {
                    for (unsigned int c = 0; c < cols; ++c) {
                        ...
                        unsigned short alt = EncodeAltitude(*ground_altitude, 
                                    ground_alt_min_, ground_alt_interval_);
                        p_high[r * cols + c] = alt / 256;
                        p_low[r * cols + c] = alt % 256;
                    }
...
unsigned int PyramidLossyMapMatrixHandler::GetBinarySize(
                            const BaseMapMatrix* base_matrix) {
    const PyramidMapMatrix* matrix = 
                dynamic_cast<const PyramidMapMatrix*>(base_matrix);
    ...
    unsigned int target_size = sizeof(unsigned int) * 2 
    ...
    for (unsigned int l = 0; l < resolution_num; ++l) {
        ...
        if (matrix->HasIntensityVar()) {
            target_size += sizeof(unsigned short) *  matrix_size;
        }
        if (matrix->HasAltitude()) {
            target_size += sizeof(unsigned short) *  matrix_size;
        }
...
unsigned int PyramidLosslessMapMatrixHandler::LoadBinary(
                                            const unsigned char* buf, 
                                            BaseMapMatrix* base_matrix) {
    PyramidMapMatrix* matrix = 
                dynamic_cast<PyramidMapMatrix*>(base_matrix);
...
unsigned int PyramidLosslessMapMatrixHandler::CreateBinary(
                                    const BaseMapMatrix* base_matrix, 
                                    unsigned char * buf, unsigned int buf_size) {
    const PyramidMapMatrix* matrix = 
                dynamic_cast<const PyramidMapMatrix*>(base_matrix);
...
unsigned int PyramidLosslessMapMatrixHandler::GetBinarySize(
                                        const BaseMapMatrix* base_matrix) {
    const PyramidMapMatrix* matrix = 
                dynamic_cast<const PyramidMapMatrix*>(base_matrix);
