...
void PyramidMapNode::Init(const BaseMapConfig* map_config) {
    ...
    for (unsigned int i = 0; i < resolutions_mr_.size(); ++i) {
        resolutions_mr_[i] = pm_map_config->map_resolutions_[0] * 
...
bool PyramidMapNode::AddValueIfInBound(
    ...
    if (is_success) {
        PyramidMapMatrix* map_matrix = dynamic_cast<PyramidMapMatrix*>(map_matrix_);
        map_matrix->AddSampleBase(static_cast<float>(intensity), coordinate[2],
                                y, x, level);
        return true;
    }
    else {
...
bool PyramidMapNode::GetCoordinate(const Eigen::Vector2d& coordinate,
    ...
    int off_x = static_cast<int>((coordinate[0] - left_top_corner_[0]) 
                                / current_resolution);
    int off_y = static_cast<int>((coordinate[1] - left_top_corner_[1]) 
                                / current_resolution);
    if (off_x >= 0 && off_x < map_matrix->GetCols(level) 
            && off_y >= 0 && off_y < map_matrix->GetRows(level)) {
...
Eigen::Vector2d PyramidMapNode::GetCoordinate(
                    unsigned int level, unsigned int x, 
                    unsigned int y) const {
    const PyramidMapConfig* map_config = 
                        dynamic_cast<const PyramidMapConfig*>(map_config_);

    float current_resolution = map_config->map_resolutions_[0] * 
                        std::pow(map_config->resolution_ratio_, level);

    Eigen::Vector2d coord;
    coord[0] = left_top_corner_[0] + current_resolution * x;
    coord[1] = left_top_corner_[1] + current_resolution * y;
    return coord;
}
...
float PyramidMapNode::GetIntensitySafe(
                        const Eigen::Vector3d& coordinate,
...
float PyramidMapNode::GetIntensityVarSafe(
                        const Eigen::Vector3d& coordinate,
                        unsigned int level) const {
    Eigen::Vector2d coord2d(coordinate[0], coordinate[1]);
    unsigned int x = 0; 
...
float PyramidMapNode::GetAltitudeSafe(
                        const Eigen::Vector3d& coordinate,
                        unsigned int level) const {
    Eigen::Vector2d coord2d(coordinate[0], coordinate[1]);
    unsigned int x = 0; 
...
float PyramidMapNode::GetAltitudeVarSafe(
                        const Eigen::Vector3d& coordinate,
                        unsigned int level) const {
    Eigen::Vector2d coord2d(coordinate[0], coordinate[1]);
    unsigned int x = 0; 
...
float PyramidMapNode::GetGroundAltitudeSafe(
                        const Eigen::Vector3d& coordinate,
                        unsigned int level) const {
    Eigen::Vector2d coord2d(coordinate[0], coordinate[1]);
    unsigned int x = 0; 
...
unsigned int PyramidMapNode::GetCountSafe(
                        const Eigen::Vector3d& coordinate,
                        unsigned int level) const {
    Eigen::Vector2d coord2d(coordinate[0], coordinate[1]);
    unsigned int x = 0; 
...
float PyramidMapNode::GetIntensity(
                        const Eigen::Vector3d& coordinate,
                        unsigned int level) const {
    Eigen::Vector2d coord2d(coordinate[0], coordinate[1]);
    unsigned int x = 0; 
...
float PyramidMapNode::GetIntensityVar(
                        const Eigen::Vector3d& coordinate,
                        unsigned int level) const {
    Eigen::Vector2d coord2d(coordinate[0], coordinate[1]);
    unsigned int x = 0; 
...
float PyramidMapNode::GetAltitude(
                        const Eigen::Vector3d& coordinate,
                        unsigned int level) const {
    Eigen::Vector2d coord2d(coordinate[0], coordinate[1]);
    unsigned int x = 0; 
...
float PyramidMapNode::GetAltitudeVar(
                        const Eigen::Vector3d& coordinate,
                        unsigned int level) const {
    Eigen::Vector2d coord2d(coordinate[0], coordinate[1]);
    unsigned int x = 0; 
...
float PyramidMapNode::GetGroundAltitude(
                        const Eigen::Vector3d& coordinate,
                        unsigned int level) const {
    Eigen::Vector2d coord2d(coordinate[0], coordinate[1]);
    unsigned int x = 0; 
...
unsigned int PyramidMapNode::GetCount(
                        const Eigen::Vector3d& coordinate,
                        unsigned int level) const {
    Eigen::Vector2d coord2d(coordinate[0], coordinate[1]);
    unsigned int x = 0; 
...
unsigned int PyramidMapNode::GetGroundCount(
                        const Eigen::Vector3d& coordinate,
                        unsigned int level) const {
    Eigen::Vector2d coord2d(coordinate[0], coordinate[1]);
    unsigned int x = 0; 
