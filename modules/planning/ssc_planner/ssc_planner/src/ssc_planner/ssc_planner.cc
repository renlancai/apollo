...
ErrorType SscPlanner::RunOnce() {
  ...
  is_lateral_independent_ =
      initial_state_.velocity > cfg_.planner_cfg().low_speed_threshold() // 2.0 mps
          ? true
          : false;
...
ErrorType SscPlanner::StateTransformForInputData() {
  ...
  {
    for (int i = 0; i < surround_forward_trajs_.size(); ++i) { // multi-agents?
      for (auto it = surround_forward_trajs_[i].begin();
           it != surround_forward_trajs_[i].end(); ++it) { // each agent's mult-model trajs?
        for (int k = 0; k < it->second.size(); ++k) { // for each traj?
          // states
